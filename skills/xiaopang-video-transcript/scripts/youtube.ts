...
/**
 * YouTube 字幕提取工具
 * 来自 baoyu-skills/baoyu-youtube-transcript
 * 支持多语言、翻译、章节分割、说话人识别
 */
...
export async function processYouTube(videoIdOrUrl: string, options?: Partial<Options>): Promise<YouTubeTranscriptResult> {
  ...
  const opts: Options = {
    ...
    excludeGenerated: options?.excludeGenerated || false,
    excludeManual: options?.excludeManual || false,
    output: options?.output || "",
    outputDir: options?.outputDir || "",
    timestamps: options?.timestamps ?? true,
    chapters: options?.chapters ?? true,
...
function printHelp() {
  console.log(`Usage: bun youtube.ts <video-url-or-id> [options]

Options:
  --languages <codes>          Language codes, comma-separated (default: zh,en)
  ...
  --chapters                   Chapter segmentation from description (default: on)
...
async function main() {
  ...
  for (const videoId of opts.videoIds) {
    try {
      const r = await processYouTube(videoId, opts);
