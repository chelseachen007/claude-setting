---
name: card-caster
description: "Content caster (铸). Transforms content into PNG visuals. Five molds: -l (default) long reading card, -i infograph, -m multi-card reading cards (1080x1440), -v visual sketchnote, -c comic (manga-style B&W). Output to ~/Downloads/. Use when user says '铸', 'cast', '做成图', '做成卡片', '做成信息图', '做成海报', '视觉笔记', 'sketchnote', '漫画', 'comic', 'manga'."
user_invocable: true
version: "1.6.0"
---
...
```bash
node ~/.claude/skills/card-caster/assets/capture.js <html> <png> <width> <height> [fullpage]
```

依赖：`~/.claude/skills/card-caster/node_modules/` 中的 playwright。如报错：

```bash
cd ~/.claude/skills/card-caster && npm install playwright && npx playwright install chromium
```
