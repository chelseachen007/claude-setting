#!/usr/bin/env node

const path = require('path');

async function main() {
  const args = process.argv.slice(2);
  const htmlPath = args[0];
  const outputPath = args[1];
  const width = parseInt(args[2]) || 1080;
  const height = parseInt(args[3]) || 1440;

  if (!htmlPath || !outputPath) {
    console.error('Usage: node capture.js <html> <png> [width] [height]');
    process.exit(1);
  }

  // Resolve playwright from global npm install
  const globalModules = require('child_process').execSync('npm root -g', { encoding: 'utf8' }).trim();
  const Module = require('module');
  const originalResolve = Module._resolveFilename;
  Module._resolveFilename = function(request, parent) {
    if (request === 'playwright') {
      return originalResolve.call(this, require('path').join(globalModules, 'playwright'), parent);
    }
    return originalResolve.call(this, request, parent);
  };

  let chromium;
  try {
    chromium = require('playwright').chromium;
  } catch {
    console.error('Playwright not found. Run: npm install -g playwright && npx playwright install chromium');
    process.exit(1);
  }

  const browser = await chromium.launch();
  const page = await browser.newPage();

  // 3x device pixel ratio for crisp output on retina
  await page.setViewportSize({ width, height });

  const fileUrl = 'file://' + path.resolve(htmlPath);
  await page.goto(fileUrl, { waitUntil: 'networkidle' });
  await page.waitForTimeout(500);

  await page.screenshot({
    path: path.resolve(outputPath),
    type: 'png',
    clip: { x: 0, y: 0, width, height }
  });

  await browser.close();
  console.log('OK: ' + path.resolve(outputPath));
}

main().catch(err => {
  console.error(err.message);
  process.exit(1);
});
