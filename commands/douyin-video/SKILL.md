...
- uv 包管理器: `brew install uv` (自动管理 Python 依赖)
...
uv run ~/.claude/commands/douyin-video/scripts/douyin_downloader.py --link "抖音分享链接" --action info

# 下载视频到指定目录
uv run ~/.claude/commands/douyin-video/scripts/douyin_downloader.py --link "抖音分享链接" --action download --output ./videos

# 提取视频文案并保存到文件 (需要 API_KEY 环境变量)
uv run ~/.claude/commands/douyin-video/scripts/douyin_downloader.py --link "抖音分享链接" --action extract --output ./output

# 提取文案并同时保存视频
uv run ~/.claude/commands/douyin-video/scripts/douyin_downloader.py --link "抖音分享链接" --action extract --output ./output --save-video

# 安静模式 (减少输出)
uv run ~/.claude/commands/douyin-video/scripts/douyin_downloader.py --link "抖音分享链接" --action extract --output ./output --quiet
```
