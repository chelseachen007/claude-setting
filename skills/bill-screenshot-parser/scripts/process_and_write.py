...
CATEGORY_KEYWORDS = {
    '饮食': ['外卖', '餐厅', '美团', '饿了么', '肯德基', '麦当劳', '面馆', '海底捞',
            ...
            '巴比', '牛肉汤', '馅饼', '鸡排', '米线', '食品', '麻辣烫', '烧鹅',
            '黄焖鸡', '沙县', '包子', '馒头', '煎饼', '水饺', '馄饨', '面条',
            '蜜雪冰城', '老娘舅', '古茗', '塔斯汀', '卷饼', '猪脚饭', '鸡公煲',
            '汉堡', '温州小吃', '披萨', '凉皮', '冒菜', '寿司', '拉面'],
    '食材': ['盒马', '超市', '菜场', '生鲜', '菜市场'],
    '住房': ['房租', '物业', '水电', '燃气', '宽带', '水费', '电费'],
    '咖啡': ['咖啡', 'Coffee', '星巴克', '瑞幸', 'Manner'],
    '交通': ['打车', '滴滴', '地铁', '公交', '加油', '停车', '高德', '充电',
            '出行', '中石化', '石化', '交通', '交通出行', '市民卡', '一码通'],
    '通讯': ['话费', '移动', '联通', '电信', '通讯'],
    '服饰': ['服装', '衣服', '鞋', 'ZARA', '优衣库'],
    ...
    '娱乐': ['电影', '游戏', '音乐', '视频', '爱奇艺', '腾讯视频', 'B站',
            'BUFF', 'steam', '腾讯天游', '娱乐', 'PY市场', '匹歪'],
    '健身': ['健身', '运动', '瑜伽', '游泳', '体育馆'],
    '日用': ['便利', '十足', '购', '日用', '洗护', '纸巾', '京东', '拼多多', '淘宝'],
...
def get_or_create_bill(work_dir, records=None):
    """获取或创建账单文件，根据记录日期确定月份"""
    if records:
        # 从记录中找最新日期来确定月份
        latest = max((r.get('date', '') for r in records), default='')
        if latest:
            try:
                dt = datetime.strptime(latest[:10], '%Y-%m-%d')
                month_name = f"{dt.year}年{dt.month}月账单.xlsx"
            except ValueError:
                today = datetime.now()
...
def main():
    ...
    # 6. 获取/创建账单文件
    bill_path, mode = get_or_create_bill(args.work_dir, processed)
